...
//        byte[] bufferData = IOUtils.toByteArray(fileInputStream, 1024);
...
//                                data[6] = new String(wordFreqs.readLine());
//                                data[6] = wordFreqs.readLine().trim();
                                if (StringUtils.isEmpty(wordFreq.trim())) {
                                    break;
                                }
                                data[6] = wordFreq.trim();
                                data[6] = new String(wordFreq.getBytes("UTF-8"), "UTF-8");
                                String tmpString = ((String) data[6]).trim().split(",")[1];
                                data[7] = Integer.parseInt(tmpString);
//                                data[7] = Integer.parseInt(((String) data[6]).split(",")[1]);
                                // 공백 문자가 없는 단어
...
//                              word_freqs.writelines("%20s,%04d\n" % (data[5], data[7]))
        ...
        // 입력 파일 처리를 마쳤다
        // FileUtils에서 close 해줌
        // TODO: word_freqs.flush()
        wordFreqs.close();
        ...
        while (true) {
            ...
            for (int i=0 ; i<25; i++) {
//                int count = Integer.parseInt(((String)data[i]).split(",")[1]);
